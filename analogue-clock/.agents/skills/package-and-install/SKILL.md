...
description: >-
  Use this skill to compile GSettings schemas, package the analogue-clock GNOME Shell extension into a ZIP file, or install and enable it locally.
---

# Package and Install Skill

This skill provides step-by-step procedures to validate, package, and locally install the `clock@devilbd.com` GNOME Shell extension.
...
- **Output:** Produces `clock@devilbd.com.shell-extension.zip`.
- **Underlying Command:**
  ```bash
  gnome-extensions pack --force --extra-source=prefs.ui --out-dir="."
  ```
...
1. Cleans and recreates `~/.local/share/gnome-shell/extensions/clock@devilbd.com`.
2. Copies `extension.js`, `metadata.json`, `stylesheet.css`, `prefs.js`, `prefs.ui`, and `schemas/`.
3. Compiles the installed schemas.
4. Enables the extension via `gnome-extensions enable clock@devilbd.com`.
